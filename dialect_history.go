...
const nameHistorySQL = "history"

func init() {
	AvailableDialects = append(AvailableDialects, nameHistorySQL)
	urlParser[nameHistorySQL] = urlParserHistoryDB
	finalizer[nameHistorySQL] = finalizerHistoryDB
	newConnection[nameHistorySQL] = newHistoryDB
}
...
func newHistoryDB(deets *ConnectionDetails) (dialect, error) {
	...
	dialectName := deets.Options["dialect"]
	if dialectName == "" {
		return nil, errors.New("history dialect requires underlying dialect option")
	}
	delete(deets.Options, "dialect")

	factory, ok := newConnection[dialectName]
	if !ok {
		return nil, errors.Errorf("unknown dialect %v", dialectName)
	}

	deets.Dialect = dialectName
	err := deets.Finalize()
	if err != nil {
		return nil, err
	}
	dialect, err := factory(deets)
	if err != nil {
		return nil, err
	}

	deets.Driver = dialect.DefaultDriver()
...
func urlParserHistoryDB(cd *ConnectionDetails) error {
	dialect := cd.Dialect
	defer func() { cd.Dialect = dialect }()
	cd.URL = strings.TrimLeft(cd.URL, "history:")
	cd.Dialect = ""
	err := cd.withURL()
	if err != nil {
		return err
	}
	cd.Options["dialect"] = cd.Dialect
	return nil
}

func finalizerHistoryDB(cd *ConnectionDetails) {
}
